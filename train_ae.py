...
def setup_args():
    ...
    options.add_argument(
        "-bs", action="store", dest="batch_size", default=1024, type=int
    )
    options.add_argument(
        "-ds",
        action="store",
        dest="datadir",
        default="data/nuclear_crops_all_experiments/",
    )

    options.add_argument(
        "-iter", action="store", dest="max_iter", default=200, type=int
    )
    ...
    options.add_argument(
        "-lamb", action="store", dest="lamb", default=0.0000001, type=float
    )
    options.add_argument(
        "-lamb2", action="store", dest="lamb2", default=0.001, type=float
    )
...
if args.conditional:
    optimizer = optim.Adam(
        list(model.parameters()) + list(netCondClf.parameters()), lr=args.lr
    )
else:
...
def train(epoch):
    ...
    with open(os.path.join(args.save_dir, "log.txt"), "a") as f:
        print(
            "Epoch: {} Average loss: {:.15f} Clf loss: {:.15f} ".format(
                epoch,
                train_loss / len(train_loader.dataset),
                total_clf_loss / len(train_loader.dataset),
            ),
            file=f,
        )
...
def test(epoch):
    ...
    with open(os.path.join(args.save_dir, "log.txt"), "a") as f:
        print(
            "Test set loss: {:.15f} Test clf loss: {:.15f}".format(
                test_loss, total_clf_loss
            ),
            file=f,
        )
...
def generate_image(epoch):
    ...
    for i in range(5):
        ...
        imageio.imwrite(
            os.path.join(img_dir, "Train_epoch_%s_inputs_%s.jpg" % (epoch, i)),
            np.uint8(inputs.cpu().data.view(64, 64).numpy() * 255),
        )
        imageio.imwrite(
            os.path.join(img_dir, "Train_epoch_%s_recon_%s.jpg" % (epoch, i)),
        ...
        imageio.imwrite(
            os.path.join(img_dir, "Test_epoch_%s_inputs_%s.jpg" % (epoch, i)),
            np.uint8(inputs.cpu().data.view(64, 64).numpy() * 255),
        )
        imageio.imwrite(
            os.path.join(img_dir, "Test_epoch_%s_recon_%s.jpg" % (epoch, i)),
            np.uint8(recon_inputs.cpu().data.view(64, 64).numpy() * 255),
        )
