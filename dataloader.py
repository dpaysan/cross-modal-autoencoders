...
class ToTensorNormalize(object):
    ...
    def __call__(self, sample):
        ...
        image_tensor = cv2.resize(
            image_tensor, dsize=(64, 64), interpolation=cv2.INTER_CUBIC
        )
...
class NucleiDatasetNew(Dataset):
    ...
    def load_images(self):
        ...
        label_data_2 = pd.read_csv(
            os.path.join(self.datadir, "protein_ratios_full.csv")
        )
        label_data = label_data.merge(label_data_2, how="inner", on="Label")
        label_dict = {
            name: (float(ratio), np.abs(int(cl) - 2))
            for (name, ratio, cl) in zip(
                list(label_data["Label"]),
                list(label_data["Cor/RPL"]),
                list(label_data["mycl"]),
            )
        }
        label_dict_2 = {
            name: np.abs(int(cl) - 2)
            for (name, cl) in zip(
                list(label_data_2["Label"]), list(label_data_2["mycl"])
            )
        }
        del label_data
        del label_data_2
        ...
        for f in os.listdir(os.path.join(self.datadir, "images")):
            ...
            if basename in label_dict.keys():
                images_test.append(
                    {
                        "name": basename,
                        "label": label_dict[basename][0],
                        "image_tensor": np.float32(io.imread(fname)),
...
class ATAC_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return {
            "tensor": torch.from_numpy(atac_sample).float(),
            "binary_label": int(cluster),
        }

    def _load_atac_data(self):
        data = pd.read_csv(
            os.path.join(self.datadir, "df_peak_counts_names_nCD4_seuratnorm.csv"),
            index_col=0,
        )
        data = data.transpose()
        labels = pd.read_csv(
            os.path.join(
                self.datadir, "clustlabels_peak_counts_names_nCD4_seurat_n_2.csv"
            ),
            index_col=0,
        )
...
class RNA_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return {
            "tensor": torch.from_numpy(rna_sample).float(),
            "coro1a": coro1a,
            "rpl10a": rpl10a,
            "label": coro1a / rpl10a,
            "binary_label": int(cluster),
    ...
    def _load_rna_data(self):
        data = pd.read_csv(
            os.path.join(
                self.datadir, "filtered_lognuminorm_pc_rp_7633genes_1396cellsnCD4.csv"
            ),
            index_col=0,
        )
        data = data.transpose()
        labels = pd.read_csv(
            os.path.join(self.datadir, "labels_nCD4_corrected.csv"), index_col=0
        )
...
def print_nuclei_names():
    dataset = NucleiDatasetNew(
        datadir="data/nuclear_crops_all_experiments", mode="test"
    )
...
def test_nuclei_dataset():
    dataset = NucleiDatasetNew(
        datadir="data/nuclear_crops_all_experiments", mode="train"
    )
    print(len(dataset))
    sample = dataset[0]
