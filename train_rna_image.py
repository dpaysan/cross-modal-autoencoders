...
def setup_args():
    ...
    options.add_argument(
        "--save-freq", action="store", dest="save_freq", default=20, type=int
    )
    options.add_argument("--pretrained-file", action="store")

    # training parameters
    options.add_argument(
        "-bs", "--batch-size", action="store", dest="batch_size", default=32, type=int
    )
    options.add_argument(
        "-w", "--num-workers", action="store", dest="num_workers", default=10, type=int
    )
    options.add_argument(
        "-lrAE",
        "--learning-rate-AE",
        action="store",
        dest="learning_rate_AE",
        default=1e-4,
        type=float,
    )
    options.add_argument(
    ...
    options.add_argument(
        "-e", "--max-epochs", action="store", dest="max_epochs", default=1000, type=int
    )
    options.add_argument(
        "-wd",
        "--weight-decay",
    ...
    options.add_argument("--beta", action="store", default=1.0, type=float)
...
image_loader = torch.utils.data.DataLoader(
    image_dataset, batch_size=args.batch_size, drop_last=True, shuffle=True
)
genomics_loader = torch.utils.data.DataLoader(
    genomics_dataset, batch_size=args.batch_size, drop_last=True, shuffle=True
)

# setup optimizer
opt_netRNA = optim.Adam(list(netRNA.parameters()), lr=args.learning_rate_AE)
opt_netClf = optim.Adam(
    list(netClf.parameters()), lr=args.learning_rate_D, weight_decay=args.weight_decay
)
...
def train_autoencoders(
    rna_inputs, image_inputs, rna_class_labels=None, image_class_labels=None
):
    ...
    if args.conditional_adv:
        rna_class_labels, image_class_labels = (
            rna_class_labels.cuda(),
            image_class_labels.cuda(),
        )
        rna_scores = netClf(
            torch.cat(
                (rna_latents, rna_class_labels.float().view(-1, 1).expand(-1, 10)),
                dim=1,
            )
        )
        image_scores = netClf(
            torch.cat(
                (image_latents, image_class_labels.float().view(-1, 1).expand(-1, 10)),
                dim=1,
            )
        )
    else:
    ...
    kl_loss = compute_KL_loss(rna_mu, rna_logvar) + compute_KL_loss(
        image_mu, image_logvar
    )
    clf_loss = 0.5 * criterion_classify(
        rna_scores, image_labels
    ) + 0.5 * criterion_classify(image_scores, rna_labels)
    loss = args.alpha * (rna_recon_loss + image_recon_loss) + clf_loss + kl_loss

    if args.conditional:
        clf_class_loss = 0.5 * criterion_classify(
            rna_class_scores, rna_class_labels
        ) + 0.5 * criterion_classify(image_class_scores, image_class_labels)
    ...
    summary_stats = {
        "rna_recon_loss": rna_recon_loss.item() * rna_scores.size(0),
        "image_recon_loss": image_recon_loss.item() * image_scores.size(0),
        "clf_loss": clf_loss.item() * (rna_scores.size(0) + image_scores.size(0)),
    }

    if args.conditional:
        summary_stats["clf_class_loss"] = clf_class_loss.item() * (
            rna_scores.size(0) + image_scores.size(0)
        )
...
def train_classifier(
    ...
    # compute losses
    clf_loss = 0.5 * criterion_classify(
        rna_scores, rna_labels
    ) + 0.5 * criterion_classify(image_scores, image_labels)
    ...
    summary_stats = {
        "clf_loss": clf_loss * (rna_scores.size(0) + image_scores.size(0)),
        "rna_accuracy": accuracy(rna_scores, rna_labels),
        "rna_n_samples": rna_scores.size(0),
        "image_accuracy": accuracy(image_scores, image_labels),
        "image_n_samples": image_scores.size(0),
    }
...
def generate_image(epoch):
    ...
    for i in range(5):
        ...
        imageio.imwrite(
            os.path.join(img_dir, "epoch_%s_trans_%s.jpg" % (epoch, i)),
            np.uint8(recon_inputs.cpu().data.view(64, 64).numpy() * 255),
        )
        recon_images, _, _, _ = netImage(image_inputs)
        imageio.imwrite(
            os.path.join(img_dir, "epoch_%s_recon_%s.jpg" % (epoch, i)),
...
for epoch in range(args.max_epochs):
    ...
    for idx, (rna_samples, image_samples) in enumerate(
        zip(genomics_loader, image_loader)
    ):
